...
type CounterData = {
	...
	deltaBasePosition: UDim2?,
	deltaTween: Tween?,
	deltaConnection: RBXScriptConnection?,
}
...
local counterState: CounterState = {
	coins = {
		total = 0,
		animator = nil,
		valueLabel = nil,
		deltaLabel = nil,
		deltaBasePosition = nil,
		deltaTween = nil,
		deltaConnection = nil,
	},
	points = {
...
local function createAnimator(tween: Tween?, connection: RBXScriptConnection?, valueObject: Instance?): Animator
	local createAnimator = require(ReplicatedStorage:WaitForChild("Shared"):WaitForChild("UI"):WaitForChild("Animator"))

	local animator = createAnimator(tween, connection, valueObject)
	local animator: Animator = {
		tween = tween,
		connection = connection,
		valueObject = valueObject,
		destroy = function()
			if animator.tween then
				animator.tween:Cancel()
			end
			if animator.connection then
				animator.connection:Disconnect()
				animator.connection = nil
			end
			if animator.valueObject then
				animator.valueObject:Destroy()
				animator.valueObject = nil
			end
		end
	}
...
local function playDelta(counter, delta: number)
	...
	local basePosition: UDim2 = counter.deltaBasePosition or label.Position
	local targetPosition = basePosition + UDim2.new(0, 0, 0, -14)
...
local function createCounters()
	...
	-- Assuming createRow returns two TextLabel instances
	...
	counterState.coins.animator = createAnimator(nil, nil, nil) -- We'll improve this in next step
	...
	counterState.points.animator = createAnimator(nil, nil, nil)
...
local function createHudGui()
	...
	local applyPalette = PaletteUtils.applyPalette
