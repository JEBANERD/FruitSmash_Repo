...
local playerStates = setmetatable({}, { __mode = "k" }) :: { [Player]: PlayerState }
local fruitStates = setmetatable({}, { __mode = "k" }) :: { [BasePart]: FruitState }
...
local function safeRequire(instance: Instance?)
    ...
    if not ok then
        warn(string.format("[CombatServer] Failed to require %s: %s", instance:GetFullName(), result))
        return nil
    end
...
local function computeFruitMaxHP(fruit: BasePart, stats: FruitStats?)
...
local function ensureFruitState(fruit: BasePart, stats: FruitStats?)
    local state = fruitStates[fruit]
    if state then
        return state
    end

    local maxHP = computeFruitMaxHP(fruit, stats)
    state = { currentHP = maxHP, maxHP = maxHP }
    fruitStates[fruit] = state

    fruit.AncestryChanged:Connect(function(instance, parent)
...
local function resolveArenaOwnershipValid(fruit: BasePart, player: Player): boolean
    local ancestor = fruit.Parent
    while ancestor do
        if ancestor:IsA("Model") then
            local arenaId = ancestor:GetAttribute("ArenaId")
            if arenaId then
...
local function notifyEconomy(player: Player, coins: number, points: number, stats: FruitStats?)
    ...
    if economy then
        ...
        if typeof(handler) == "function" then
            ...
            if not ok then
                warn(string.format("[CombatServer] Economy reward failed: %s", err))
            end
        end
    end
...
local function applyDurabilityWear(player: Player, amount: number, stats: FruitStats?)
    ...
    if economy and typeof(economy.ApplyMeleeDurabilityWear) == "function" then
        ...
        warn(string.format("[CombatServer] ApplyMeleeDurabilityWear failed: %s", err))
    end
