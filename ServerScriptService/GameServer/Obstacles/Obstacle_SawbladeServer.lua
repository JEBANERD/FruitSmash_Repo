...
local function bladeLoop(state, blade)
	while state.running do
		-- If obstacle system is disabled for current level, sleep a bit.
		if not state.enabled then
			task.wait(1)
		...
		-- Wait a random interval before popping up
		...
		-- If we were stopped/disabled during the wait, skip this cycle
