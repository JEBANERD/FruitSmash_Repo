...
local function sanitizeSeconds(value: any): number?
...
local function sanitizePercent(value: any): number?
	local numeric = tonumber(value)
	if not numeric then
		return nil
	end

	if numeric > 1 and numeric <= 100 then
		numeric /= 100
	end
...
local function assign(target: { [string]: any }, key: string, value: any, aliases: { string }?)
	if value ~= nil then
		target[key] = value
		if aliases then
